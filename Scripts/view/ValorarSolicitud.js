$(document).ready(function () {
    ...
    //LEER COOKIES JQUERY
    //var Gerencia = getCookieValues("slnGerencia", "codnive");
    //var Dependencia = getCookieValues("slnGerencia", "coddepe");
    //alert(getCookie("slnRecursosHumanos"));
    //alert(getCookie("slnGerencia"));
    ...
    //LEER COOKIES JQUERY
    //function getCookieValues(cname,cparam) {
    //    var name = cname + "=";
    //    var ca = document.cookie.split(';');
    //    for (var i = 0; i < ca.length; i++) {
    //        var c = ca[i];
    ...
    //        }
    //        if (c.indexOf(name) == 0) {
    //            var params = c.substring(name.length, c.length).split("&");                 
    //        }
    //    }
    //    var param = cparam + "=";
    ...
    //            cad = cad.substring(1);
    //        }
    //        if (cad.indexOf(param) == 0) {
    //            return cad.substring(param.length, cad.length);
    //        }
    //    }
    ...
    function cargarGrilla() {
        $("#grdSolicitud").jqGrid({
            ...
            rowNum: 5, rowList: [5, 10, 15, 20], pager: "#pageSolicitud", sortname: "CODI_EMPL_PER", viewrecords: true, sortorder: "asc",
            height: 285,
            autowidth: true,
            shrinkToFit: false,
    ...
    $("#btnAprobar").click(function () {
        ...
        if (selRowIds.length > 0) {
            if (confirmar("el proceso de Aprobar")) {
                var sJson = "";
    ...
    $("#btnDesaprobar").click(function () {
        ...
        if (selRowIds.length == 1) {
            if (confirmar("el proceso de Desaprobar")) {
    ...
    function ShowModalAutenticacion() {
        ...
        for (var rowId in selectedRows) {
            ...
            var dato = JSON.stringify({
                TIPO_PLAN_TPL: $("#grdSolicitud").jqGrid("getCell", rowId, "TIPO_PLAN_TPL"),
                CODI_EMPL_PER: $("#grdSolicitud").jqGrid("getCell", rowId, "CODI_EMPL_PER"),
                NOMB_EMPL_PER: $("#grdSolicitud").jqGrid("getCell", rowId, "NOMB_EMPL_PER"),
                SECU_PROG_DDF: $("#grdSolicitud").jqGrid("getCell", rowId, "SECU_PROG_DDF"),
                ANIO_GENE_DFI: $("#grdSolicitud").jqGrid("getCell", rowId, "ANIO_GENE_DFI"),
                INIC_PROG_DDF: new Date(FehIni[2], FehIni[1] - 1, FehIni[0]),
                FINA_PROG_DDF: new Date(FehFin[2], FehFin[1] - 1, FehFin[0]),
        ...
        sJson = "[" + dato + "]";
        $("#modalDesaprueba").load(requrlModalAutenticacion + "?token=" + token + "&flagPrimeraCarga=" + 0 + "&sJson=" + encodeURIComponent(sJson),
            function () {                
                $("#modalDesaprueba").modal("show");
            });
    }
